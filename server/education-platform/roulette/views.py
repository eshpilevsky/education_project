...
from .models import Match, Meeting, Request, StudentRequest, TeacherRequest
from .serializers import (MatchSerializer, MeetingSerializer,
                          StudentRequestSerializer, TeacherRequestSerializer)
...
class MatchUserMixin(object):
    ...
    def get_object(self):
        ...
        if obj:
            ...
            raise exceptions.NotFound(detail="No request found!")


class MatchTypeMixin(object):
    """
    Matches type argument to be either student/teacher
    """
    ...
    def get_queryset(self):
        ...
        if type == 'student':
            ...
            return TeacherRequest.objects.all()
        else:
...
class UserBelongsToMatch(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        if type == 'student':
            ...
            return obj.teacher_request.user == request.user
        else:
            return False
...
class AccessPermission(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if type == 'student':
            ...
            if hasattr(request.user, 'teacherdata') and request.user.teacherdata.verified:
                return True
        return type is None
...
class MeetingListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return self.queryset.filter(Q(student=self.request.user) | Q(teacher=self.request.user))
...
class ReportListView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if meeting.student and meeting.teacher:
            provider = self.request.user
            receiver = meeting.teacher if provider == meeting.student else meeting.student
            serializer.save(provider=provider, receiver=receiver)
        else:
            raise ValidationError("Meeting must have students and teachers")
...
type_parameter = openapi.Parameter('type', openapi.IN_PATH, description='User type',
                                   required=True, type=openapi.TYPE_STRING, enum=['student', 'teacher'], )
...
def match_answer(request, uuid, type):
    ...
    elif type == 'teacher' and match.teacher_request.user == request.user:
        # user is teacher
        match.teacher_agree = agree
        if not agree:
            match.delete()
        else:
            match.save()
...
def join_meeting(request, match_uuid):
    ...
    if match:
        ...
        if hasattr(match, 'meeting'):
            ...
            elif user == match.teacher_request.user:

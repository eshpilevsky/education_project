...
class MatchSerializer(DynamicFieldsModelSerializer):
    #teacher_uuid = serializers.ReadOnlyField(source='teacher_request.user.uuid')
    teacher = CustomUserSerializer(source='teacher_request.user', read_only=True)
    ...
    class Meta:
        model = Match
        fields = ['uuid', 'student_agree', 'teacher_agree',
...
class MeetingSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['meeting_id', 'ended',
                  'time_ended', 'student', 'teacher', 'name', 'feedback_set']

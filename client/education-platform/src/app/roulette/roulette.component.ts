...
export class RouletteComponent implements OnInit, OnDestroy {
  ...
  // type === 'teacher'=> invoke teacher component
  type: UserType;
  state: State;
  join: JoinResponse;
  meeting: Meeting;
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.endsWith("student")) {
      this.type = "student";
    } else if (this.router.url.endsWith("teacher")) {
      this.type = "teacher";
    } else {
      this.type = "student";
    }
    ...
    if (this.state === "create" && this.type === "teacher") {
      constants$ = this.teacherMatchCreate(constants$);
    }
    constants$.subscribe();
  }

  teacherMatchCreate(obs: Observable<{ constants: Constants }>) {
    return obs
      .pipe(take(1))
      .pipe(
        switchMap((data) => {
          // TODO: is this valid?
          const subj = this.authenticationService.currentUserValue.teacherdata
            .subjects[0].id;

...
export class TeacherFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.register) {
      ...
      this.f.img.setValue(this.user.teacherdata.profilePicture);
      this.f.subjects.setValue(
        this.subjects.map((s) =>
          this.user.teacherdata.subjects.map((s) => s.id).includes(s.id)
        )
      );
      ...
      this.f.schoolTypes.setValue(
        this.schoolTypes.map(
          (s) =>
            this.user.teacherdata.schooldata.filter(
              (x) => x.schoolType.id == s.id
            ).length > 0
        )
      );
      this.existingProfile = this.user.teacherdata.profilePicture;
      ...
      this.f.sliders.setValue(
        this.schoolTypes.map((type) => {
          ...
          let userFiltered = this.user.teacherdata.schooldata.filter(
